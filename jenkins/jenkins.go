...
func (jenkins *Jenkins) buildBaseUrl(path string, params url.Values) (requestUrl string) {
...
func (jenkins *Jenkins) getQuery(path string, params url.Values, body interface{}) (err error) {
	requestUrl := jenkins.buildBaseUrl(path, params)
	req, err := http.NewRequest("GET", requestUrl, nil)
	if err != nil {
		return
	}
...
// FuzzyJobName 模糊查询
func (jenkins *Jenkins) FuzzyJobName(param string) (names []string, err error) {
	var searchResult SearchResult
	err = jenkins.getQuery(fmt.Sprintf("/search/suggest?query=%s", param), nil, &searchResult)
...
// GetJobConfig 返回一个 Maven 作业，该作业具有用于创建 Maven 作业的作业
...
// GetBuild 返回指定作业的第 number 个 build 结果。
...
// GetComputer 返回具有指定名称的 Computer 对象。
...
// hasParams 返回一个布尔值，该值指示作业是否已参数化
